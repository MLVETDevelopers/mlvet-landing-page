...
namespace CMS {
    ...
    export const landingPageContentBlocks = async (): Promise<
        ContentBlockData[]
    > => {
        const res = await fetch(
            `${base}/landing-page?populate[0]=contentBlock&populate[1]=contentBlock.media`
        )
        const data = await res.json()
    ...
    export const downloadPage = async (): Promise<DownloadPageContent> => {
        const res = await fetch(
            `${base}/download-page?populate[0]=osDownloadUrl&populate[1]=osDownloadUrl.osLogo`
        )
        const data = await res.json()
    ...
    export const helpDocCategories = async (): Promise<
        HelpDocArticlesCategory[]
    > => {
